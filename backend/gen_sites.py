...
# Hard bucket: every product has an "Add to wishlist" DECOY button BEFORE its
# "Add to cart" button. wish(id) files to the wishlist (does NOT reach the cart),
# so a mis-click fails the goal. window.__STATE__ carries cart AND wishlist so the
# verifier / MCP browser_evaluate can tell a trap-click from a real add.
_TRAP_TEMPLATE = """<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  <title>DemoShop ({n} items)</title>
...
<body>
  ...
  <ul id="grid" class="grid"></ul>
  <div id="cart">Cart: <span id="cart-items"></span></div>
  <div id="wishlist">Wishlist: <span id="wishlist-items"></span></div>
  <footer>
    <div>About</div><div>Careers</div><div>Privacy Policy</div><div>Terms</div>
    <div>Returns</div><div>Contact</div>
    <p>&copy; 2026 DemoShop Inc. Free shipping over $50.</p>
  ...
  <script>
    ...
    window.__CART__ = [];
    window.__WISHLIST__ = [];
    window.__STATE__ = {{ cart: window.__CART__, wishlist: window.__WISHLIST__, products: window.__PRODUCTS__ }};
    function add(id) {{
      window.__CART__.push(id);
      document.getElementById('cart-items').textContent = window.__CART__.join(', ');
    }}
    function wish(id) {{
      window.__WISHLIST__.push(id);
      document.getElementById('wishlist-items').textContent = window.__WISHLIST__.join(', ');
    }}
    const grid = document.getElementById('grid');
    window.__PRODUCTS__.forEach(function (p) {{
      const li = document.createElement('li');
      ...
      li.innerHTML =
        '<span class="name">' + p.name + '</span> - ' +
        '<span class="price">$' + p.price + '</span> (' + p.color + ') ' +
        '<button id="wish-' + p.id + '" onclick="wish(\\'' + p.id + '\\')">Add to wishlist</button> ' +
        '<button id="add-' + p.id + '" onclick="add(\\'' + p.id + '\\')">Add to cart</button>';
      grid.appendChild(li);
    }});
  </script>
...
def main() -> None:
    ...
    for n in TRAP_SIZES:
        products = build_products(n)
        html = _TRAP_TEMPLATE.format(n=n, products_json=json.dumps(products))
        out = _SITES_DIR / f"shop_trap_{n}" / "index.html"
        out.parent.mkdir(parents=True, exist_ok=True)
        out.write_text(html)
